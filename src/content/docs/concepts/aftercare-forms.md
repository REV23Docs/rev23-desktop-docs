...
sidebar:
  ...
  order: 12
---
Aftercare Forms are Templates that are linked to the **Aftercare Form** property of a [Service Type](/rev23-desktop-docs/configuration/service-types/). Aftercare can be printed from a service, sale, or emailed automatically to the customer by the Virtual Receptionist after a service has been completed.
...
You can customize the subject of the email that is sent in [**Configuration > Custom Texts**](/rev23-desktop-docs/configuration/custom-texts/) by modifying the custom text with key name `WORKFLOW_AFTERCARE_EMAIL_SUBJECT`.
...
Additionally, you can be prompted to print aftercare instructions after a sale containing one or more services has been fully paid. You can disable this prompt, or set it to always automatically print in [Configuration: My Studio: Options: Service Options](/rev23-desktop-docs/configuration/my-studio/#service-options).
