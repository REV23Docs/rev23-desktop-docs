...
**Clone:** In some views, a Clone action will appear. Think of the clone action like copy and paste for objects. Creating a clone will take the selected object's values and create a new object with those existing values in place. This is particularly useful for inventory items where you want to create a variant of a single item.
...
When you refresh in a list view, the list will be refreshed from the database. For example, if you have add-on PCs connecting to the REV23 Desktop database, one PC has the Customer List View open, and a new customer is added on another PC, you can execute the Refresh action on the first PC, and that customer will now appear in the Customer List View.
...
Imagine this: you've started a new Service and selected a customer. The customer is clearly not a minor, but the service requires a guardian to be present. The problem is that the customer's birthday has either been entered incorrectly or not entered at all in their customer profile. You can highlight the Customer property in the Service record and then click the Open Related Record button. This will open that customer's profile, where you can correct their birthday, then save and close that record. Now, the service is no longer requesting a guardian, and you can proceed.
...
This is useful if a customer alerts you that they did not receive their email appointment reminder. You can find their appointment, execute this action, and see whether that workflow ran. If it Completed, they should have received the reminder; if it Aborted, there was a problem when the workflow ran.

## Records Navigation

**Previous Record/Next Record:** The Previous Record/Next Record actions allow you to navigate between objects in the database. In a detail view, this is useful when trying to find or inspect certain data. You can open the detail view for the first object, then use the Next Record action to load the next database object into your currently loaded detail view.
